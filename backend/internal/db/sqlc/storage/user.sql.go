...
func (q *Queries) ClearTelegramBlocked(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, clearTelegramBlocked, id)
	return err
}
...
const listDailyReminderCandidates = `-- name: ListDailyReminderCandidates :many
...
type ListDailyReminderCandidatesRow struct {
	ID          uuid.UUID
	TelegramID  pgtype.Int8
	Username    string
	DisplayName pgtype.Text
...
func (q *Queries) ListDailyReminderCandidates(ctx context.Context, limit int32) ([]ListDailyReminderCandidatesRow, error) {
	rows, err := q.db.Query(ctx, listDailyReminderCandidates, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListDailyReminderCandidatesRow
	for rows.Next() {
		var i ListDailyReminderCandidatesRow
		if err := rows.Scan(
			&i.ID,
			&i.TelegramID,
			&i.Username,
...
func (q *Queries) MarkDailyReminderSent(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markDailyReminderSent, id)
	return err
}

const markTelegramBlocked = `-- name: MarkTelegramBlocked :exec
UPDATE users SET telegram_blocked_at = now() WHERE id = $1 AND telegram_blocked_at IS NULL
`

func (q *Queries) MarkTelegramBlocked(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markTelegramBlocked, id)
	return err
}
